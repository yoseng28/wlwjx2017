...
<table width="80%" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    <td height=30><div align="center"><strong><font color="#FF0000">下载管理</font></strong></div></td>
  </tr>
</table>
<table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr> 
    <td height=5><table width="602" border="1" cellpadding="2" cellspacing="1" bordercolor="#376ed0">
        <form name="form2" method="post" action="download_info.asp">
          <tr> 
            <td width="90" height=30 align="center"><strong>搜索</strong></td>
            <td width="217"> 标题： 
              <input name="keyword" type="text" id="keyword2"></td>
            <td width="119" align="center"> <input type="submit" name="Submit" value=" 搜 索 "> 
            </td>
            <td width="145" align="center"><a href="download_info.asp"><font color="#FF0000">查看所有内容</font></a></td>
          </tr>
        </form>
      </table></td>
  </tr>
...
<table width="95%" border="1" align="center" cellpadding="2" cellspacing="1" bordercolor="#376ed0">
 <form name="form1" method="post" action="download_infodel.asp"> <tr> 
...
if keyword<>"" then
sql="select * from download Where DownName Like '%"& keyword &"%' order by id desc"
else
...
%>
  <tr bgcolor="#f9f9f9"> 
    ...
    <td bgcolor="#f9f9f9"> &nbsp; 
      <a href="../downloadview.asp?id=<%=rs("id")%>" target="_blank" title="<%=rs("DownName")%>"> 
      <% =rs("DownName") %>
      </a></td>
    <td align="center">&nbsp;<%=rs("DownUser")%></td>
    <td align="center">&nbsp;<%=rs("DownBigClass")%> </td>
    <td align="center">&nbsp;<%=rs("DownSmallClass")%></td>
    <td align="center">&nbsp;<%=rs("DownTime")%></td>
    <td align="center"><a href="download_infomodi.asp?id=<%=rs("id")%>">修改</a> 
      <a href="download_infodel.asp?id=<%=rs("id")%>" onClick="return ConfirmDelnr();">删除</a></td>
  </tr>
  <%
rs.movenext
...
<%if Page<2 then      
    ...
    response.write "<a href=download_info.asp?page=1>首页</a>&nbsp;"
    response.write "<a href=download_info.asp?page=" & Page-1 & ">上一页</a>&nbsp;"
  end if
  if rs.pagecount-page<1 then
    response.write "下一页 尾页"
  else
    response.write "<a href=download_info.asp?page=" & (page+1) & ">"
    response.write "下一页</a> <a href=download_info.asp?page="&rs.pagecount&">尾页</a>"
  end if
...
<%if Page<2 then      
    ...
    response.write "<a href=download_info.asp?keyword="&keyword&"&page=1>首页</a>&nbsp;"
    response.write "<a href=download_info.asp?keyword="&keyword&"&page=" & Page-1 & ">上一页</a>&nbsp;"
  end if
  if rs.pagecount-page<1 then
    response.write "下一页 尾页"
  else
    response.write "<a href=download_info.asp?keyword="&keyword&"&page=" & (page+1) & ">"
    response.write "下一页</a> <a href=download_info.asp?keyword="&keyword&"&page="&rs.pagecount&">尾页</a>"
  end if
   response.write "&nbsp;页次：<strong><font color=red>"&Page&"</font>/"&rs.pagecount&"</strong>页 "
    response.write "&nbsp;共<b><font color='#FF0000'>"&rs.recordcount&"</font></b>条记录 <b>"&rs.pagesize&"</b>条记录/页"
%>
